...
- `aeon.gp.profile.v1` — AEON GP profile contract, including strict/reserved-only defaults, collection semantics for `list`/`tuple`, container semantics for `object`/`node`, datatype semantics for `radix`/`decimal` and `sep`/`separator`/`kadot`, and required processor capabilities
