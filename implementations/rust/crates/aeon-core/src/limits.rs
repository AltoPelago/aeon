...
///
/// Identity and profile claims are descriptive metadata. They are kept
/// separate from the normalized codec and finalization option subsets.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct EffectiveTelexConfiguration {
    pub limits_id: String,
    pub limits_version: String,
    pub profile_claims: Vec<String>,
    pub telex: TelexLimits,
    pub finalization: FinalizationLimits,
...
pub fn telex_limits(limits: &AeonicLimitsV1) -> Result<TelexLimits, LimitsDiagnostic> {
    Ok(TelexLimits {
        max_input_bytes: bounded(
            limits.telex.max_input_bytes,
            67_108_864,
            1_073_741_824,
            "max_input_bytes",
        )?,
        max_line_bytes: bounded(
            limits.telex.max_line_bytes,
            1_048_576,
            67_108_864,
            "max_line_bytes",
        )?,
        max_fields_per_event: bounded(
            limits.telex.max_fields_per_event,
            64,
            4_096,
            "max_fields_per_event",
        )?,
        max_events: bounded(
            limits.processing.max_events,
            100_000,
            1_000_000,
            "max_events",
        )?,
        max_decoded_payload_bytes: bounded(
            limits.telex.max_decoded_payload_bytes,
            33_554_432,
            1_073_741_824,
...
pub fn effective_telex_configuration(
    limits: &AeonicLimitsV1,
) -> Result<EffectiveTelexConfiguration, LimitsDiagnostic> {
    Ok(EffectiveTelexConfiguration {
        limits_id: limits.limits_id.clone(),
        limits_version: limits.limits_version.clone(),
        profile_claims: limits.profile_claims.clone(),
...
mod tests {
    use super::{
        LimitSetting, aeon_compile_limits, effective_telex_configuration, finalization_limits,
...
"#;
    ...
    fn loads_and_normalizes_closed_v1_limits() {
        ...
        let effective_telex =
            effective_telex_configuration(&limits).expect("effective Telex configuration");
        assert_eq!(effective_telex.limits_id, "altopelago.aeonic-limits.v1");
        assert_eq!(effective_telex.limits_version, "1.0.0");
        assert_eq!(effective_telex.profile_claims, ["aeon.gp.profile.v1"]);
        assert_eq!(effective_telex.telex, telex);
        assert_eq!(effective_telex.finalization, finalize);
        assert!(!effective_telex.overrides_applied);
    }
