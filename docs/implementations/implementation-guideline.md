...
- CLI commands should not invent semantics beyond what the library layers already define.
  ...
  Keep extending fixture coverage into already-supported modes. Rust still found useful gaps once it started pinning recovery-mode `inspect`, strict typed/untyped toggle fixtures, symbolic-reference rendering, and formatter stdin behavior, even though the underlying features already existed.
