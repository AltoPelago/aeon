...
| `bench-sofia-python.py` | Measures an installed exact or `abi3-py312` native Python wheel against matching native Rust, separating JSON, packed, native-object, conservative conversion, ergonomic, and encoded-Telex work. | `python3 ./scripts/bench-sofia-python.py --variant exact --wheel /tmp/wheel.whl --output /tmp/python-boundary.json` |
